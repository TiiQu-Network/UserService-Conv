...
public class AddUserToGroupHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent input, Context context) {
        ...
        try {
            ...
            JsonObject addUserToGroupDetails = JsonParser.parseString(requestBody).getAsJsonObject();
            JsonObject addUserToGroupResult = userService.addUserToGroup(userPoolId, addUserToGroupDetails);

            logger.log("original response body::: " + addUserToGroupResult);

            boolean isSuccessful = addUserToGroupResult.get("isSuccessful").getAsBoolean();
            int statusCode = addUserToGroupResult.get("statusCode").getAsInt();
            if (isSuccessful) {
                ...
                FailureResponse failureResponse = FailureResponse.builder()
                        .isSuccessful(isSuccessful)
                        .message(addUserToGroupResult.get("statusMessage").getAsString())
                        .build();
                response.withStatusCode(statusCode);
