...
public class SendVerificationCodeHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent input, Context context) {
        ...
        try {
            ...
            JsonObject confirmationRequest = JsonParser.parseString(requestBody).getAsJsonObject();
            String username = confirmationRequest.get("username").getAsString();
            JsonObject verificationCodeResponse = userService.resendVerificationCode(appClientId, appClientSecret, username);
            logger.log("original response body::: " + "contains sensitive information");

            boolean isSuccessful = verificationCodeResponse.get("isSuccessful").getAsBoolean();
            int statusCode = verificationCodeResponse.get("statusCode").getAsInt();
            ...
            if (isSuccessful) {
                ...
                var failureResponse = FailureResponse.builder()
                        .isSuccessful(isSuccessful)
                        .message(verificationCodeResponse.get("statusMessage").getAsString())
                        .build();
                response.withStatusCode(statusCode);
