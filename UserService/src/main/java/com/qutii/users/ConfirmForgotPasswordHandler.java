...
public class ConfirmForgotPasswordHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent input, Context context) {
        ...
        try {
            ...
            logger.log("original request body::: " + "contains sensitive information");

            JsonObject confirmForgotPasswordRequest = JsonParser.parseString(requestBody).getAsJsonObject();
            JsonObject confirmForgotPasswordResult = userService.confirmForgotPassword(appClientId, appClientSecret,
                    confirmForgotPasswordRequest);
            logger.log("original response body::: " + confirmForgotPasswordResult);

            boolean isSuccessful = confirmForgotPasswordResult.get("isSuccessful").getAsBoolean();
            int statusCode = confirmForgotPasswordResult.get("statusCode").getAsInt();
            ...
            if (isSuccessful) {
                ...
                var failureResponse = FailureResponse.builder()
                        .isSuccessful(isSuccessful)
                        .message(confirmForgotPasswordResult.get("statusMessage").getAsString())
                        .build();
                response.withStatusCode(statusCode);
