...
public class ConfirmUserHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent input, Context context) {
        ...
        try {
            ...
            JsonObject confirmSignupResult = userService.confirmUserSignup(appClientId, appClientSecret, email, code);
            logger.log("original response body::: " + confirmSignupResult);
            boolean isSuccessful = confirmSignupResult.get("isSuccessful").getAsBoolean();
            int statusCode = confirmSignupResult.get("statusCode").getAsInt();
            if (isSuccessful) {
                ...
                var failureResponse = FailureResponse.builder()
                        .isSuccessful(isSuccessful)
                        .message(confirmSignupResult.get("statusMessage").getAsString())
                        .build();
                response.withStatusCode(statusCode);
